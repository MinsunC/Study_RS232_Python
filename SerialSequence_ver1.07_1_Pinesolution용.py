...
class SerialCommunication:
    ...
    def __init__(self):
        ...
        self.button2.grid(row=1, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)

        # 구분선
        self.divide = Label(window, text="-------------------------------\n Cycle test")
        self.divide.grid(row=2, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)
        ...
        self.caution.grid(row=11, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)
        ...
        self.caution.grid(row=30, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)
        ...
        self.text3.grid(row=90, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)

        # Target cycle 수 표기
        self.text4 = Label(window, text="Target cycle: " + str(targetcycle // 10000) + " x 10,000 + " + str(targetcycle % 10000))
        self.text4.grid(row=91, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)

        # Sequence 진행 button
        # startcycle
        self.button2 = Button(window, text="Start cycle", command=lambda: self.startcycle())
        self.button2.grid(row=100, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)

        # Cycle stop button
        # close
        self.exit = Button(window, text="Stop cycle", command=lambda: self.close())
        self.exit.grid(row=110, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)

        # Reset button
        # resetcycle
        self.reset = Button(window, text="Reset", command=lambda: self.resetcycle())
        self.reset.grid(row=120, column=0, padx=2, pady=2, columnspan=2, ipadx=2, ipady=2)
        ...
        try:
            ...
            if starttriger == 1:
                ...
                if cycle == 0:
                    while True:
                        ...
                        try:
                            ...
                            for m in range(len(command)):
                                ...
                                # Pinesolution: set point A에 position setting 한 뒤 set point A 활성화
                                else:
                                    # set point A position control mode(T 1 0)
                                    buffer1 = set1poscontrol + tail
                                    ser.write(buffer1.encode('ascii'))
                                    # set point 1 position 값 setting(S 1 입력값)
                                    buffer = 'S 1 ' + command[m] + tail
                                    ser.write(buffer.encode('ascii'))
                                    # set point 1 활성화(D1)
                    ...
                    # target cycle로 자연수가 입력 되었을 경우 진행하는 code 진행 내용은 위와 동일
                    while i < cycle:
                        if exitcondition:
                            ser.write(cl.encode('ascii'))
                            break
