...
class LogFilter:
    ...
    def __init__(self):
        ...
        self.button1.grid(row=0, column=0, columnspan=2, padx=17, pady=7, ipadx=3, ipady=3)

        self.button2 = Button(window, text="Selection: Save directory", command=lambda: self.savedir())
        self.button2.grid(row=10, column=0, columnspan=2, padx=17, pady=7, ipadx=3, ipady=3)

        self.explain1 = Label(window, text="원하는 column만 남기기")
        self.explain1.grid(row=11, column=0, padx=17, pady=7, ipadx=3, ipady=3)
    ...
    def logfiles(self):
        ...
        try:
            ...
            if fileext == '.xls' or fileext == '.xlsx':
                ...
                with open(loglists, 'rt', encoding='UTF-8') as f:
                    for line in f:
    ...
    def startcyclearrange(self, col, stepcol):
        ...
        for row in range(df.shape[0]):
            ...
            if df[stepcol][row] == 1 and df[stepcol][row] != previousstep:
                cyclecount += 1
            ...
            columndata_buffer.append(df[col][row])
            previousstep = df[stepcol][row]
