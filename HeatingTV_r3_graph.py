        ...
        for file in file_list:
            ...
            with open(filename, 'rt', encoding='UTF-8') as f:
                ...
                for j in range(len(dep_pos)):
                    if len(dep_pos) != 1:
                        print("cycle", j+1)
                        result_dep.append(round(numpy.mean(dep_pos[j]), 2))
                        result_dep.append(round(numpy.std(dep_pos[j]), 2))
                        result_dep.append(numpy.max(dep_pres[j]))
                        result_dep.append(numpy.min(dep_pres[j]))

                        result_nit.append(round(numpy.mean(nit_pos[j]), 2))
                        result_nit.append(round(numpy.std(nit_pos[j]), 2))
                ...
                result_pwr[0].append(round(numpy.mean(heaterpwr[0]), 2))
                result_pwr[0].append(numpy.max(heaterpwr[0]))
                result_pwr[0].append(numpy.min(heaterpwr[0]))
                result_pwr[1].append(round(numpy.mean(heaterpwr[1]), 2))
